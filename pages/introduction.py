...
layout = html.Div(
    [
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalBody(html.Ul([
                    html.Li("Bert is a transformer-based machine learning algorithm that was designed to pre-train deep bidirectional representations from natural language text by masking a percentage of the input tokens and training the network to predict the masked tokens."),
                    html.Li("BERT has been shown to be very successful at a wide range of natural language processing tasks such as question answering, text classification, and language modeling."),
                    html.Li(
                    ...
                    html.Li("The model can be fine-tuned for specific tasks by adding a task-specific layer on top of the pre-trained model. For example, to fine-tune Bert for text classification, a task-specific classification layer can be added.",),
                    html.Li(["BERT is a very powerful model that has achieved state-of-the-art results on a wide range of natural language processing tasks.", html.A("[2]", id="bert-ref2", href="#references"), ])]),),
            ],
            id="modal-scroll",
            is_open=False,
        ),
        dbc.Offcanvas(
            html.Ul([
                ...
                html.Li(
                    "In NSP, the model is trained to predict whether two sentences are consecutive or not.",),
                html.Li("The model can be fine-tuned for specific tasks by adding a task-specific layer on top of the pre-trained model. For example, to fine-tune Bert for text classification, a task-specific classification layer can be added.",),
                html.Li(["BERT is a very powerful model that has achieved state-of-the-art results on a wide range of natural language processing tasks.",
        ...
        dbc.Offcanvas(
            html.Ul([
                html.Li("Transfer Learning is a technique that can be used to improve the performance of a machine learning model by using knowledge from a related task. The main idea is to transfer knowledge from a model that has been trained on a large and high-quality dataset to a model that is trained on a smaller and lower-quality dataset."),
        ...
        html.Div(html.Img(src="https://editor.analyticsvidhya.com/uploads/444861_2vse-G3FKMT2D59NFWduMA.jpg", className="img_tl",
                 title="https://editor.analyticsvidhya.com/uploads/444861_2vse-G3FKMT2D59NFWduMA.jpg"), className="img_holder"),
        ...
        html.P("The BERT model is a great example of a model that can be used for transfer learning. BERT was originally trained on a large corpus of English text. We can then use this model to perform tasks such as text classification, question answering, and machine translation."),
        html.P("There are many factors to keep in mind when using transfer learning with the BERT model. In the next sections we will discuss about them in some details."),
        html.Hr()
    ],
